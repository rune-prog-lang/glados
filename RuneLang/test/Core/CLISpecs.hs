...
runCLIActionTests =
  testGroup
    ...
    , testCase "runCLI (CompileAll file Nothing) hits compile and uses default 'out'" $ do
        result <- try (runCLI (CompileAll "dummy-compile-default.ru" Nothing)) :: IO (Either SomeException ())
        case result of
          Right _ -> return ()
          Left _ -> return ()

    , testCase "runCLI (CompileAll file Just out) hits compile and uses specified output" $ do
        result <- try (runCLI (CompileAll "dummy-compile-specified.ru" (Just "specified.bin"))) :: IO (Either SomeException ())
        case result of
          Right _ -> return ()
          Left _ -> return ()
    ]

  ...
  [ testCase "collects functions and overrides" $
    ...
    testCase "later definitions override earlier entries" $
      let stack = either error id (findFunc shadowProgram)
       in do
          HM.lookup "i32_dup_i32" stack @?= Just ((TypeI32, [Parameter "value" TypeI32 Nothing]), Public, False)
    ...
    testCase "struct methods are also collected" $
      findFunc structMethodProgram @?= (Right $ HM.fromList [("i32_Vec_len_Vec",((TypeI32,[Parameter "self" (TypeCustom "Vec") Nothing]),Public,False))]),
    ...
    testCase "accepts function definition with array of any type (RELOADED)" $
      let stack = either error id (findFunc arrayOverrideProgram)
       in HM.lookup "null_show_arrany" stack @?= Just ((TypeNull, [Parameter {paramName = "arr", paramType = TypeArray TypeAny, paramDefault = Nothing}]), Public, False)
  ]
...
findDefsTests = testGroup "findDefs"
  [ testCase "inserts new function definition" $
      case findDefs HM.empty (DefFunction "add" [Parameter "a" TypeI32 Nothing, Parameter "b" TypeI32 Nothing] TypeI32 [] False Public False False) of
        Right stack -> do
          case HM.lookup "i32_add_i32_i32" stack of
            ...
            Just (t, _, _) -> assertFailure $ "Found function but return type mismatch. Expected i32, got " ++ show t
        Left err -> assertFailure $ "Expected success but got: " ++ err,
    testCase "rejects duplicate function with same signature" $
      case findDefs (HM.singleton "i32_add_i32_i32" ((TypeI32, [Parameter "a" TypeI32 Nothing, Parameter "b" TypeI32 Nothing]), Public, False)) (DefFunction "add" [Parameter "a" TypeI32 Nothing, Parameter "b" TypeI32 Nothing] TypeI32 [] False Public False False) of
    ...
    testCase "appends override to existing function" $
      case findDefs (HM.singleton "show" ((TypeNull, [Parameter "value" TypeAny Nothing]), Public, False)) (DefFunction "show" [Parameter "x" TypeI32 Nothing] TypeNull [] False Public False False) of
        Right stack -> do
    ...
    testCase "rejects struct with duplicate method names" $
      case findDefs HM.empty (DefStruct "Vec" [] [DefFunction "len" [Parameter "self" TypeAny Nothing] TypeI32 [] False Public False False, DefFunction "len" [Parameter "self" TypeAny Nothing] TypeI32 [] False Public False False] False Nothing) of
        Left err -> "FuncAlreadyExist:" `isInfixOf` err @? "Expected FuncAlreadyExist error"
...
transformStructMethodsTests = testGroup "transformStructMethods"
  [ testCase "prefixes method names with struct name" $
      let methods = [DefFunction "len" [Parameter "self" TypeAny Nothing] TypeI32 [] False Public False False]
          transformed = transformStructMethods "Vec" methods
       in case transformed of
            [DefFunction name _ _ _ _ _ _ _] -> name @?= "Vec_len"
...
duplicateFunctionProgram =
  Program
    "duplicate-func"
    [ DefFunction "foo" [Parameter "x" TypeI32 Nothing] TypeI32 [] False Public False False,
      DefFunction "foo" [Parameter "y" TypeF32 Nothing] TypeF32 [] False Public False False
    ]
...
arrayOverrideProgram =
  Program
    "array-override"
    [ DefFunction "show" [Parameter "arr" (TypeArray TypeAny) Nothing] TypeNull [] False Public False False
    ]

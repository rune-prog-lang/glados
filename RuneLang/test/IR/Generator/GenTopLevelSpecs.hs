...
testGenTopLevel = testGroup "genTopLevel"
  ...
  , testCase "Routes DefOverride to genOverride" $
      let def = DefOverride "Point_show" [Parameter "self" (TypeCustom "Point") False] TypeNull [] False
          result = runGenUnsafe (genTopLevel def)
      in case result of
...
testGenFunction = testGroup "genFunction"
  ...
  , testCase "Generates function with params" $
      let def = DefFunction "add" [Parameter "a" TypeI32 False, Parameter "b" TypeI32 False] TypeI32 [] False
          result = runGenUnsafe (genTopLevel def)
      in case result of
        [IRFunctionDef func] -> do
          irFuncName func @?= "add"
...
testGenOverride = testGroup "genOverride"
  [ testCase "genOverride with params" $
      let def = DefOverride "print" [Parameter "x" TypeI32 False] TypeNull [] False
          result = runGenUnsafe (genTopLevel def)
      in case result of
...
testGenStruct = testGroup "genStruct"
  ...
  , testCase "Generates struct with methods" $
      let def = DefStruct "Vec2" [Field "x" TypeF32] 
                [DefFunction "Vec2_magnitude" [Parameter "self" (TypeCustom "Vec2") False] TypeF32 [] False]
          result = runGenUnsafe (genTopLevel def)
      in do
        length result @?= 2
        case result of
...
testGenStructMethod = testGroup "genStructMethod"
  [ testCase "Handle generation for method name" $
      let method = DefFunction "Point_calc" [Parameter "self" (TypeCustom "Point") False] TypeI32 [] False
...
testGenParam = testGroup "genParam"
  [ testCase "Generates param with i32 type" $
      let param = Parameter "x" TypeI32 False
  ...
  , testCase "Converts struct type to pointer" $
      let param = Parameter "point" (TypeCustom "Point") False
      in case evalState (runExceptT (genParam param)) emptyState of
        Left err -> assertFailure $ "Unexpected error: " ++ err
        Right result -> result @?= ("p_point", IRPtr (IRStruct "Point"))

  , testCase "Handles primitive types" $
      let param = Parameter "flag" TypeBool False
      in case evalState (runExceptT (genParam param)) emptyState of
        Left err -> assertFailure $ "Unexpected error: " ++ err

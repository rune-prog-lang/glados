...
testSelectReturnType = testGroup "selectReturnType"
  [ testCase "Returns correct return type for existing function" $
      let fs = HM.fromList [("i32_f_i32_f32", ((TypeI32, [Parameter "a" TypeI32 Nothing, Parameter "b" TypeF32 Nothing]), Public))]
          args = [IRI32, IRF32]
      in runGenUnsafe (selectReturnType fs "f" args) @?= IRI32
  
  , testCase "Maps IR types back to AST types correctly" $
      let fs = HM.fromList [("u32_g_u32_ptr_i32_S", ((TypeU32, [Parameter "a" TypeU32 Nothing, Parameter "b" (TypePtr TypeI32) Nothing, Parameter "c" (TypeCustom "S") Nothing]), Public))]
          args = [IRU32, IRPtr IRI32, IRPtr (IRStruct "S")]
      in runGenUnsafe (selectReturnType fs "g" args) @?= IRU32 
  , testCase "Select signature Nothing" $
      let fs = HM.fromList [("f64_h_f32", ((TypeF64, [Parameter "x" TypeF32 Nothing]), Public))]
          args = [IRF32]
      in runGenUnsafe (selectReturnType fs "h" args) @?= IRF64
  , testCase "Returns Left for no matching signature" $ do
        let fs = HM.fromList [("i32_mismatched_i32_f32", ((TypeI32, [Parameter "a" TypeI32 Nothing, Parameter "b" TypeF32 Nothing]), Public))]
            funcName = "mismatched"
